  ...
  async executeOpenscad(
    ...
    try {
      ...
      if (error instanceof Error) {
        throw new Error('Kele exited with an error: ' + error.message);
      } else {
        throw new Error('Kele exited with an error');
      }
    }
    ...
    if (exitCode === 0) {
      try {
        ...
        if (error instanceof Error) {
          throw new Error('Kele cannot read created file: ' + error.message);
        } else {
          throw new Error('Kele cannot read created file');
        }
      }
